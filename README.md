...
**`--nightly` (experimental — unsupported):** `install_fizgig_rocm.bat --nightly` installs unpinned `torch[device-ARCH]` / `torchvision[device-ARCH]` / `rocm-sdk-devel` from the same multi-arch index and leaves `BNB_ROCM_VERSION` unset so bitsandbytes auto-selects its highest matching DLL. This path is for local experimentation only. **Do not open GitHub issues for crashes, install failures, or training problems when `--nightly` was used** — those reports will not be supported. Use the pinned install (no flag) for anything you expect help with.
